...
public class DescribeConsumerGroupsHandler extends AdminApiHandler.Batched<CoordinatorKey, ConsumerGroupDescription> {
    ...
    public DescribeGroupsRequest.Builder buildBatchedRequest(int coordinatorId, Set<CoordinatorKey> keys) {
        ...
        return new DescribeGroupsRequest.Builder(groupIds);
    }
    ...
    public ApiResult<CoordinatorKey, ConsumerGroupDescription> handleResponse(
        ...
        for (String groupId : response.groups().keySet()) {
            DescribeGroupsResponse.GroupMetadata meta = response.groups().get(groupId);
            CoordinatorKey groupIdKey = CoordinatorKey.byGroupId(groupId);
            Errors error = Errors.forCode(meta.errorCode());
            if (error != Errors.NONE) {
                handleError(groupIdKey, error, failed, groupsToUnmap);
                continue;
            }
            final String protocolType = meta.protocolType();
            if (protocolType.equals(ConsumerProtocol.PROTOCOL_TYPE) || protocolType.isEmpty()) {
                final List<DescribeGroupsResponse.GroupMember> members = meta.members();
                ...
                for (DescribeGroupsResponse.GroupMember groupMember : members) {
                    ...
                    if (groupMember.memberAssignment().hasArray()) {
                        final PartitionAssignor.Assignment assignment = ConsumerProtocol.
                                deserializeAssignment(ByteBuffer.wrap(groupMember.memberAssignment().array()));
                ...
                final ConsumerGroupDescription consumerGroupDescription =
                        new ConsumerGroupDescription(groupIdKey.idValue, protocolType.isEmpty(),
                                memberDescriptions,
                                meta.protocol(),
                                ConsumerGroupState.parse(meta.state()),
                                coordinator,
                                null);
