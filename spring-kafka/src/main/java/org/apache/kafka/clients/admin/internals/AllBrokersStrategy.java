...
public class AllBrokersStrategy implements AdminApiLookupStrategy<AllBrokersStrategy.BrokerKey> {
    ...
    public MetadataRequest.Builder buildRequest(Set<BrokerKey> keys) {
        ...
        return new MetadataRequest.Builder(new MetadataRequestData().topics().stream()
                .map(MetadataRequestData.MetadataRequestTopic::name).collect(Collectors.toList()));
    }
    ...
    public LookupResult<BrokerKey> handleResponse(Set<BrokerKey> keys, AbstractResponse abstractResponse) {
        ...
        Map<BrokerKey, Integer> brokerKeys = brokers.stream().collect(Collectors
                .toMap(broker -> new BrokerKey(OptionalInt.of(broker.id())), Node::id));
