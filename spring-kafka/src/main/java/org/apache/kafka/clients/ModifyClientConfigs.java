...
public class ModifyClientConfigs {
    private static final Logger log = LoggerFactory.getLogger(ModifyClientConfigs.class);
    ...
    public static Map<String, Object> postProcessReconnectBackoffConfigs(AbstractConfig config, Map<String, Object> parsedValues) {
        ...
        if (!config.originals().containsKey("reconnect.backoff.max.ms") && config
                .originals().containsKey("reconnect.backoff.ms")) {
            log.debug("Disabling exponential reconnect backoff because {} is set, but {} is not.", "reconnect.backoff.ms", "reconnect.backoff.max.ms");
            rval.put("reconnect.backoff.max.ms", parsedValues.get("reconnect.backoff.ms"));
        }
        return rval;
    }
}
