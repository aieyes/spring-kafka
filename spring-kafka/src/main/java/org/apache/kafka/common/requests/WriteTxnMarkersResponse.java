...
import static org.apache.kafka.common.requests.ProduceResponse.DEFAULT_THROTTLE_TIME;
        ...
        for (WritableTxnMarkerResult marker : data.markers()) {
            for (WritableTxnMarkerTopicResult topic : marker.topics()) {
                for (WritableTxnMarkerPartitionResult partitionResult : topic.partitions()) {
                    ...
                    Integer count = errorCounts.get(errors);
                    if (count == null) {
                        errorCounts.put(errors, 1);
                    } else {
                        errorCounts.put(errors, ++ count);
                    }
                }
            }
        }
